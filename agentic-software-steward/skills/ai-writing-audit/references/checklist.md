...
This checklist is adapted from Aaron Makelky's MIT-licensed `ai-writing-audit`, which in turn cites Wikipedia's Signs of AI Writing guide. Apply judgment; patterns are not proof of authorship.
...
- `[PROMO-FILLER]`: generic superlatives, category cliches, or unsupported promotional language.
- `[SUPERFICIAL-ING]`: an `-ing` phrase attached to a fact without adding substance.
- `[GENERIC-FUTURE]`: vague future-looking optimism or obligatory challenges sections.
- `[REQUIREMENTS-COPY]`: page copy that narrates implementation, authentication, APIs, page structure, or internal requirements instead of speaking to the customer.
- `[REPETITION]`: multiple sections restating the same claim without a new job.
...
- `[METADISCOURSE]`: explaining what the reader should conclude instead of providing the scene, fact, or argument.
- `[RULE-OF-3]`: repeated three-part phrasing used as automatic comprehensiveness.
- `[STACCATO]`: fragments stacked mechanically rather than serving the voice.
...
- `[HEDGING]`: aims to, strives to, may help, can support, or designed to enable when the brand can make a clearer claim.

## Structural And Formatting Patterns

- `[TITLE-CASE]`: title case used everywhere without a design reason.
...
- Product UI should prioritize clarity and action.
- Technical writing may need signposting and repeated terminology.
- Legal and regulated copy may require precise qualifiers.

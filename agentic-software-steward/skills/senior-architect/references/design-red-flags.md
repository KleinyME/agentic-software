...
Screen every sketch before committing to it. A red flag is a reason to revise or reject the shape, not a style note.
...
- Callers must coordinate several methods to complete one operation.
...
Temporal decomposition organizes modules by execution order instead of the knowledge they own. Separate load, validate, transform, and save stages usually repeat one representation and its invariants across several boundaries.
...
A pass-through method forwards the same arguments to another method of the same shape. It adds a layer without hiding complexity.

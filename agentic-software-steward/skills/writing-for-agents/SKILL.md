...
description: Writing documents an agent consumes. Use when creating or editing a skill, AGENTS.md, or a reference file one of them points at.
---
...
The bar for a skill in this repository:

- One procedure per skill. Doctrine is stated once as a principle card and cited by name, never restated in each skill that touches it.
- Default to `disable-model-invocation: true`. A model-facing description is a permanent per-turn cost, so it is earned by the agent genuinely needing to reach the skill alone.
- A model-invoked description stays under 350 characters and names both what fires it and what does not.
- A rule a script can check belongs in the script, not in prose in several skills.
